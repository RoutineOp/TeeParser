...
type
  ...
  private
    ...
    function EmitIdentifiers(const AIdents:TIdentifiers):String;
    function EmitOperator(const AExp:TOperatorExpression):String;
    function EmitPackage(const APackage:TPackage):String;
    ...
    function EmitUses(const AUses:TUses; HideSystem,UnitPath:Boolean):String;
  ...
  TPascalEmitHTML=class(TPascalEmit)
  protected
...
Constructor TPascalEmit.Create;
begin
  inherited;
  Quote:='''';
end;

function TPascalEmit.Directives(const AIdent:TIdentifier):String;
begin
  result:='';
  ...
  if AIdent.IsDeprecated then
     result:=result+' '+Keyword('deprecated');

  if AIdent.IsInline then
     result:=result+' '+Keyword('inline');
end;

function TPascalEmit.Values(const V:TArrayValues):String;
var t, H: Integer;
begin
  result:='('+CRLF;
...
function TPascalEmit.Value(const ABlock:TBlock):String;
begin
...
function TPascalEmit.TypeParams(const AParams:TTypeParameters; Constraints:Boolean=False):String;
...
function TPascalEmit.NameAndParams(const AType:TTypeDeclaration; Constraints:Boolean=False):String;
begin
  result:=AType.Name;
  ...
  if AType.Expression<>nil then
     if (AType.Expression is TGenericSpec) and
        (TGenericSpec(AType.Expression).TypeParameters<>nil) then
           result:=result+TypeParams(TGenericSpec(AType.Expression).TypeParameters);
  }
end;

function TPascalEmit.TypeName(const AType:TTypeDeclaration; Constraints:Boolean=False):String;
begin
  if AType=nil then
     result:='?nil'
  else
...
function TPascalEmit.ArrayType(const A:TArraySpecification; const Indent:String):String;
var t : Integer;
begin
  result:='';
...
function TPascalEmit.AncestorsOf(const R:TRecordSpecification; const Exclude:String):String;
var t : Integer;
    tmp : TTypeDeclaration;
    DoExclude : Boolean;
    tmpName : String;
...
function TPascalEmit.TypeSet(const AItems:TTypeDeclarations; const Indent:String):String;
var tmp,
    LastWidth,
    t : Integer;
    tmpExp : TExpression;
...
function TPascalEmit.Parameters(const P:TParameters; const Delimiter:String; const AsParams:Boolean=False):String;
var t,H : Integer;
    tmp : TParameter;
...
function TPascalEmit.MethodSpec(const ASpec:TMethodSpecification; const AName:String):String;
...
class function TPascalEmit.ModuleExtension: String;
begin
  result:='.pas';
end;

function TPascalEmit.Range(const ARange:TRangeExpression):String;
begin
...
function TPascalEmit.EmitClass(const AClass:TRecordSpecification; const Indent:String; IsForward:Boolean;
                   const AHelper:TTypeDeclaration;
                   const ADirectives:String):String;
    ...
    if tmp='' then
       ...
       begin
         if IsForward then
            ...
            result:=result+IndentText+Keyword('end')+CRLF
       end
       else
          result:=result+IndentText+Keyword('end')+SpaceAndDirectives
    else
       result:=result+CRLF+tmp+Indent+IndentText+Keyword('end')+SpaceAndDirectives;
  end;
end;

function TPascalEmit.MethodDirectives(const AMethod:TMethodSpecification):String;
...
function TPascalEmit.Attributes(const ABlock:TBlock; const Indent:String):String;
var t : Integer;
    tmp : TAttribute;
    tmpS : String;
begin
...
function TPascalEmit.TypeSpec(const AType:TTypeSpecification;
  ...
  begin
    ...
    begin
      ...
      result:=result+CRLF+Attributes(tmpSpec,Indent)+
              EmitRecord(TInterfaceSpecification(tmpSpec),Indent)+Indent+IndentText+Keyword('end');
    end;
  end
  else
...
function TPascalEmit.EmitType(const AType:TTypeDeclaration; const Indent:String):String;
begin
...
function TPascalEmit.EmitTypeSpec(const AType: TTypeDeclaration;
  const Indent: String): String;
begin
  result:=TypeSpec(AType.Expression,AType.IsForward,Indent,Directives(AType));
end;

function TPascalEmit.EmitConst(const AConst:TConstantDeclaration; const Indent:String;
                   OmitType:Boolean=False):String;
begin
...
function TPascalEmit.EmitIdentifiers(const AIdents: TIdentifiers): String;
var t,
    tmpLen,
    H : Integer;
    tmp,
...
begin
  ...
  begin
    if tmp='' then
       tmp:=IndentText; // Indent

    tmpName:=AIdents[t].Name;
    Inc(tmpLen,Length(tmpName)+1);

    tmp:=tmp+UnitNameLink(tmpName);
...
function TPascalEmit.Variables(const AVar:TVariableDeclarations; const Indent:String;
                               ThreadVars:Boolean=False):String;
var t : Integer;
...
function TPascalEmit.CallParameters(const P:TExpressions):String;
var H,t : Integer;
begin
  result:='';
...
function TPascalEmit.EmitCase(const C:TCase; const Indent:String):String;
...
var t,H : Integer;
    Next : TCaseItem;
begin
  result:=Indent+Keyword('case')+' '+Expression(C.Expression)+' '+
          Keyword('of')+' '+CRLF;
  ...
  result:=result+Indent+Keyword('end');
end;

function TPascalEmit.Expressions(const AItems:TExpressions):String;
var t, H : Integer;
begin
  result:='';
...
function TPascalEmit.EmitASM(const S:TASMStatements; const Indent:String):String;
var t : Integer;
begin
...
function TPascalEmit.Statement(const S:TStatement; const Indent:String):String;
...
begin
  ...
  if S is TWith then
  begin
    result:=Keyword('with')+' '+Expressions(TWith(S).Items)+' '+
            Keyword('do')+CRLF+
  ...
  begin
    ...
    result:=result+' '+Keyword('do')+CRLF+Statements(TForStatement(S).Loop,IndentText);
  end
  else
  if S is TTry then
  begin
    ...
    result:=result+IndentText+Keyword('end');
  end
  else
  if S is TCase then
...
function TPascalEmit.Method(const AMethod:TMethodDeclaration; Body:Boolean; const Indent:String):String;
...
begin
  ...
  begin
    result:=result+CRLF+Section(tmpSpec.Body,Body,Indent+IndentText)+
            Statements(tmpSpec.Body.Statements,Indent);
...
function TPascalEmit.Methods(const AVisibility:TVisibility;
...
function TPascalEmit.EmitProperty(const P:TPropertyDeclaration):String;
...
function TPascalEmit.CaseVariable(const ACase:TCaseVariable; const Indent:String;
                          const OmitType:Boolean):String;
var t : Integer;
...
function TPascalEmit.Fields(const AVisibility:TVisibility;
                   const AFields:TVariableDeclarations;
                   const Indent:String;
                   OmitType:Boolean=False;
                   Properties:Boolean=False):String;
...
function TPascalEmit.Constants(const AVisibility:TVisibility; const AConst:TConstantDeclarations; const Indent:String):String;
var t : Integer;
    tmp : TConstantDeclaration;
begin
  result:='';
...
function TPascalEmit.EmitRecord(const ARecord:TRecordSpecification; const Indent:String):String;
var S,
    tmp : String;
    t : Integer;
    tmpIdent : TBlock;
...
function TPascalEmit.OperatorToString(const AOperator: TOperator): String;
begin
  case AOperator of
     opEqual: result:='=';
  opNotEqual: result:=LessThan+GreaterThan;
       opAnd: result:=Keyword('and');
        opOr: result:=Keyword('or');
       opXor: result:=Keyword('xor');
       opShl: result:=Keyword('shl');
       opShr: result:=Keyword('shr');
...
opLowerEqual: result:=LessThan+'=';
  ...
  opMultiply: result:='*';
    opDivide: result:='/';
       opNot: result:=Keyword('not');
       opDiv: result:=Keyword('div');
       opMod: result:=Keyword('mod');
...
function TPascalEmit.CallExpression(const ACall:TCallExpression):String;
begin
...
function TPascalEmit.EmitOperator(const AExp:TOperatorExpression):String;
...
function TPascalEmit.Expression(const AExp:TExpression; const AOwner:TExpression=nil):String;
...
function TPascalEmit.Statements(const S:TStatements; const Indent:String; BeginEnd:Boolean=True):String;
...
begin
  ...
  begin
    if BeginEnd and S.BeginEnd then
       result:=Indent+Keyword('begin')+CRLF
    else
       result:='';
...
function TPascalEmit.Types(const AVisibility:TVisibility; const ATypes:TTypeDeclarations;
                           const Indent:String):String;
var t : Integer;
    tmp : TTypeDeclaration;
begin
...
function TPascalEmit.Section(const ASection:TSection; Bodies:Boolean;
                             const Indent:String):String;
var S,
    tmp : String;
    t : Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if OldClass=TLabelDeclaration then
           S:='label'
        else
...
function IsSystemUnit(const AUnit:TUnit):Boolean;
begin
  result:=SameText(AUnit.Name,'System') or SameText(AUnit.Name,'SysInit');
end;

function UsesMoreThanSystem(const AUses:TUses):Boolean;
var L : Integer;
begin
  L:=AUses.Count;
  result:=L>0;
  ...
  if result then
     if (L=1) and IsSystemUnit(TUnit(AUses[0])) then
        result:=False;
end;

function TPascalEmit.EmitUnit(const AUnit:TUnit):String;
begin
  result:=Keyword('unit')+' '+AUnit.Name+';'+CRLF+CRLF+
          Keyword('interface')+CRLF+CRLF+
          Block(AUnit.UnitInterface)+CRLF+
          Keyword('implementation')+CRLF+CRLF;

  if AUnit.ModuleImplementation<>nil then
     result:=result+Block(AUnit.ModuleImplementation);
  ...
  result:=result+CRLF+Keyword('end.');
end;

function TPascalEmit.EmitUses(const AUses:TUses; HideSystem,UnitPath:Boolean):String;
var t,
    tmpLen,
    H : Integer;
    tmp,
...
begin
  ...
  for t:=0 to H do
  begin
    if (not HideSystem) or (not IsSystemUnit(TUnit(AUses[t]))) then
    begin
      if tmp='' then
         tmp:=IndentText; // Indent

      tmpName:=AUses[t].Name;
      Inc(tmpLen,Length(tmpName)+1);

      tmp:=tmp+UnitNameLink(tmpName);

      if UnitPath and (AUses[t].FileName<>'') then
         tmp:=tmp+' in '+''''+AUses[t].FileName+'''';
...
function TPascalEmit.UsesSection(const ASection:TUsesSection):String;
begin
  if (ASection.UsesUnits<>nil) and UsesMoreThanSystem(ASection.UsesUnits) then
     result:=Keyword('uses')+CRLF+Block(ASection.UsesUnits)+CRLF+CRLF;

  result:=result+Section(ASection,True,'');
end;

function TPascalEmit.EmitProgram(const AProgram:TProgram):String;
begin
  result:=Keyword('program')+' '+AProgram.Name+';'+CRLF+
          Block(AProgram.ModuleImplementation)+CRLF+
          Keyword('begin')+CRLF+
          Block(AProgram.Main)+CRLF+
          Keyword('end.');
end;

function TPascalEmit.EmitPackage(const APackage:TPackage):String;
begin
  result:=Keyword('package')+' '+APackage.Name+';'+CRLF;

  if (APackage.PackageRequires<>nil) and (APackage.PackageRequires.Count>0) then
     result:=result+'requires'+CRLF+Block(APackage.PackageRequires)+CRLF;

  if (APackage.PackageContains<>nil) and (APackage.PackageContains.Count>0) then
     result:=result+'contains'+CRLF+Block(APackage.PackageContains)+CRLF;

  result:=result+Keyword('end.');
end;

function TPascalEmit.Block(const ABlock:TBlock):String;
begin
  ...
  if ABlock is TPackage then
     result:=EmitPackage(TPackage(ABlock))
  else
  if ABlock is TUsesSection then
     result:=UsesSection(TUsesSection(ABlock))
  else
  if ABlock is TUses then
     result:=EmitUses(TUses(ABlock),True,False) // not (ModuleOf(ABlock) is TUnit)
  else
  ...
  if ABlock is TPackages then
     result:=EmitIdentifiers(TPackages(ABlock))
  else
  if ABlock is TPackageContains then
     result:=EmitUses(TPackageContains(ABlock),False,True)
  else
  if ABlock=nil then
     result:='?nil'
  else
...
function TPascalEmitHTML.AnchorOf(const S: String): String;
begin
  if S='' then
     result:=''
  else
...
Constructor TPascalEmitHTML.Create;
begin
  inherited;
...
function TPascalEmitHTML.Keyword(const S: String): String;
begin
  result:='<span class="k">'+S+'</span>';
end;

function TPascalEmitHTML.LinkTo(const ABlock:TBlock; const S: String): String;
begin
...
class function TPascalEmitHTML.ModuleExtension: String;
begin
  result:='.html';
end;

class function TPascalEmitHTML.Page(const ABlock: TBlock): String;
begin
  result:='<html><head>'+DefaultStyle+'</head><body>'+AsString(ABlock)+'</body></html>';
end;

function TPascalEmitHTML.StringToCode(const S: String): String;
begin
  result:='<span class="s">'+inherited {$IFNDEF D12}StringToCode(S){$ENDIF}+'</span>';
end;

function TPascalEmitHTML.UnitNameLink(const S: String): String;
begin
  result:='<a href="'+S+'.html">'+S+'</a>';
end;

class function TPascalEmitHTML.DefaultStyle:String;
